...
Test(configuration_parser, parse_port_valid) {
    ...
    auto ports = serverConfig.getPorts();
    cr_assert_eq(ports.size(), 1, "Should have one valid port");
    cr_assert_eq(ports[0], 8080, "Should set a valid port");
}
...
Test(configuration_parser, parse_port_leading_zeros) {
    ...
    auto ports = serverConfig.getPorts();
    cr_assert_eq(ports.size(), 1, "Should have one valid port");
    cr_assert_eq(ports[0], 80, "Should correctly interpret port with leading zeros");
}
...
Test(configuration_parser, parse_port_with_spaces) {
    ...
    auto ports = serverConfig.getPorts();
    cr_assert_eq(ports.size(), 1, "Should have one valid port");
    cr_assert_eq(ports[0], 8080, "Should handle port numbers with spaces");
}
...
Test(configuration_parser, parse_multiple_valid_ports) {
    std::string line = "port=8080,80,443";
    Server serverConfig;
    ConfigurationParser::parsePort(line, serverConfig);
    auto ports = serverConfig.getPorts();
    cr_assert_eq(ports.size(), 3, "Should have three valid ports");
    cr_assert_eq(ports[0], 8080, "First port should be 8080");
    cr_assert_eq(ports[1], 80, "Second port should be 80");
    cr_assert_eq(ports[2], 443, "Third port should be 443");
}

Test(configuration_parser, parse_invalid_and_valid_ports) {
    std::string line = "port=8080,abc,443";
    Server serverConfig;
    cr_assert_throw(ConfigurationParser::parsePort(line, serverConfig), ConfigurationParser::InvalidConfigurationException, "Should throw exception for invalid port combination");
}
